...
const dashboardIo = new Server(dashboardServer, {
  path: '/$dashboard_ws'
});
...
// Nhận log từ proxy server
const dashboardLogs = [];
const emitLogToDashboard = (type, args) => {
  const message = args.map(a => (typeof a === 'object' ? JSON.stringify(a, null, 2) : a)).join(' ');
  const logData = { time: new Date().toISOString(), type, message };
  dashboardLogs.push(logData);
  dashboardIo.emit('proxy-log', logData);
};

// Gán console.* hook riêng cho dashboard
['log', 'error', 'warn', 'info', 'debug'].forEach(method => {
  const orig = console[method];
  console[method] = (...args) => {
    emitLogToDashboard(method, args);
    orig.apply(console, args);
  };
});

dashboardServer.listen(4000, () => {
  console.log('Dashboard server running at http://localhost:4000');
});
